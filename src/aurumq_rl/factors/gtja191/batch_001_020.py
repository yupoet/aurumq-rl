...
def gtja_002(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        (-1.0 * delta(base, 1)).alias("gtja_002").cast(pl.Float64)
    ).to_series()
...
def gtja_003(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        sum_(inner, 6).alias("gtja_003").cast(pl.Float64)
    ).to_series()
...
def gtja_004(panel: pl.DataFrame) -> pl.Series:
    ...
    expr = pl.when((m8 + s8) < m2).then(-1.0).otherwise(
        pl.when(m2 < (m8 - s8)).then(1.0).otherwise(
            pl.when(vol_ratio >= 1.0).then(1.0).otherwise(-1.0)
        )
    )
...
def gtja_007(panel: pl.DataFrame) -> pl.Series:
    ...
    return staged.select(
        (
            (rank(pl.col("__g007_max")) + rank(pl.col("__g007_min")))
            * rank(pl.col("__g007_dv"))
        )
        .alias("gtja_007")
        .cast(pl.Float64)
    ).to_series()
...
def gtja_008(panel: pl.DataFrame) -> pl.Series:
    ...
    return staged.select(
        rank(pl.col("__g008_d")).alias("gtja_008").cast(pl.Float64)
    ).to_series()
...
def gtja_009(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        sma(inner, 7, 2).alias("gtja_009").cast(pl.Float64)
    ).to_series()
...
def gtja_011(panel: pl.DataFrame) -> pl.Series:
    ...
    """
    base = (
        2.0 * pl.col("close") - pl.col("low") - pl.col("high")
    ) / (pl.col("high") - pl.col("low"))
    return panel.select(
...
def gtja_013(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        expr.alias("gtja_013").cast(pl.Float64)
    ).to_series()
...
def gtja_014(panel: pl.DataFrame) -> pl.Series:
    ...
    """
    return panel.select(
        delta(pl.col("close"), 5).alias("gtja_014").cast(pl.Float64)
    ).to_series()
...
def gtja_015(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        expr.alias("gtja_015").cast(pl.Float64)
    ).to_series()
...
def gtja_018(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        expr.alias("gtja_018").cast(pl.Float64)
    ).to_series()
...
def gtja_019(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        expr.alias("gtja_019").cast(pl.Float64)
    ).to_series()
...
def gtja_020(panel: pl.DataFrame) -> pl.Series:
    ...
    return panel.select(
        expr.alias("gtja_020").cast(pl.Float64)
    ).to_series()
