"""Upload Phase 20 handoff bundle to ledashi-oss/fromsz/."""
...
PREFIX = "fromsz/handoffs/2026-05-05-phase20-long-data/"
...
def main() -> int:
    ...
    handoff = ROOT / "handoffs" / "2026-05-05-phase20-long-data"
    for p in sorted(handoff.glob("*.md")):
        manifest.append((p, p.name))

    rep = ROOT / "reports" / "phase20_long_data"
    if rep.exists():
        for p in sorted(rep.glob("*")):
            manifest.append((p, f"reports/{p.name}"))

    p20 = ROOT / "runs" / "phase20_long_data"
    for fname in ("decision_log.md", "phase20_20a.train.log", "phase20_20a.eval.log",
                  "phase20_20b.train.log", "phase20_20b.eval.log"):
        p = p20 / fname
        if p.exists():
            manifest.append((p, f"reports/{fname}"))

    # Phase 20 model bundle (best ckpt + metadata)
    for run_name, ckpt_step in (
        ("phase20_20a_long_drop_mkt_seed42", 249920),
        ("phase20_20b_long_drop_mkt_seed4", 74976),
    ):
