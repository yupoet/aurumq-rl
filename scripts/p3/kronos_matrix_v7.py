"""Matrix v7 — same 7 cells on paris R2-B (232 cols, drop 108 alpha+gtja extras).

Final ablation per paris REPLY v22 decision tree:
  - If v3_MAIN_BOARD H2 fwd20 IC ≥ +4.143% (ledashi 228 baseline) → alpha+gtja
    pruning hypothesis CONFIRMED → paris promotes R2-B to production
  - If still < +4.143% → R2-C intersection (drop all 134 paris-only = 232 cols
    pure ledashi-common intersection)
"""
...
PANEL_R2B = "D:/dev/aurumq-handoffs/inbox/2026-05-15-paris-panel-v2-ablation-r2b/combined_panel_v_x_v2_R2B_minus_alpha_gtja_extras.parquet"
...
OUT_DIR = Path("data/kronos/outputs/matrix_v7")
...
def load_panel(name):
    if name == "r2b":
        print(f"[panel] reading {PANEL_R2B} ...", flush=True)
        t = time.time()
        p = pd.read_parquet(PANEL_R2B)
        p = _dt(p)
        drop_cols = [
        ...
        print(f"  panel R2-B: {len(p):,} rows × {len(p.columns)} cols ({time.time()-t:.0f}s)")
        return p
    elif name == "ledashi":
...
def main():
    ...
    v3_r2b = matrix_results.get("v3_MAIN_BOARD_r2b", {}).get("static", {}).get("H2_2025", {}).get("fwd20", {}).get("ic", float("nan"))
    v3_ledashi = matrix_results.get("v3_MAIN_BOARD_ledashi", {}).get("static", {}).get("H2_2025", {}).get("fwd20", {}).get("ic", float("nan"))
    verdict = ""
    if v3_r2b == v3_r2b and v3_ledashi == v3_ledashi:
        delta = (v3_r2b - v3_ledashi) * 100
        if v3_r2b >= v3_ledashi:
            verdict = f"R2-B_BEATS_LEDASHI Δ=+{delta:.3f}pp → alpha+gtja pruning CONFIRMED → paris promotes R2-B"
        else:
            verdict = f"R2-B_LOSES_LEDASHI Δ={delta:.3f}pp → R2-C intersection needed (drop 134 paris-only)"

    out = {
        "config": {
            "tier": "v7 — paris R2-B panel (232 cols, drop 108 alpha+gtja extras)",
            "cells": CELLS,
            "panel_r2b": PANEL_R2B,
    ...
    Path("data/kronos/outputs/matrix_v7_results.json").write_text(json.dumps(out, indent=2, default=str))

    lines = ["# Matrix v7 — paris R2-B panel ablation (232 cols, drop 108 alpha+gtja extras)", ""]
    ...
    lines.append("")
    lines.append("| cell | n_pred | H1 IC% fwd20 | H2 IC% fwd20 | Q1 IC% fwd20 | Q2 IC% fwd20 |")
    lines.append("|---|---|---|---|---|---|")
    for cell_id in matrix_results:
        ...
        lines.append(f"| {cell_id} | {r['n_pred_rows']:,} | {fmt('H1_2025')} | {fmt('H2_2025')} | {fmt('Q1_2026')} | {fmt('Q2_2026_partial')} |")

    lines.append("")
    lines.append("## Full ablation progression (v3_MAIN_BOARD H2 fwd20 IC%)")
    lines.append("")
    ...
    r2b_ic = matrix_results.get('v3_MAIN_BOARD_r2b', {}).get('static', {}).get('H2_2025', {}).get('fwd20', {}).get('ic', float('nan'))
    r2b_str = f"{r2b_ic*100:+.3f}" if r2b_ic == r2b_ic else "n/a"
    delta_str = f"{(r2b_ic - v3_ledashi)*100:+.3f}pp" if r2b_ic == r2b_ic else "n/a"
    lines.append(f"| **matrix v7** | **R2-B (drop 108 alpha+gtja)** | 232 | **{r2b_str}** | **{delta_str}** |")
    lines.append("| ledashi 228 pruned | baseline | 228 | +4.14 ⭐ | 0 |")

    Path("data/kronos/outputs/matrix_v7_table.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"\n[saved] matrix_v7_results.json + matrix_v7_table.md")
