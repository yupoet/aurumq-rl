"""Matrix v5 — same 7 cells as v4, on paris panel_v2_no_phase_c (366 cols).

Per paris REPLY v21 §5.B: ablation to test if Phase C NULL pre-2024-04 was the root
cause of panel v2 underperformance. paris dropped 13 Phase C cols + 2 Kronos cols =
366 cols total.

Expected:
- If `panel_v2_no_phase_c >= ledashi 228 pruned` on MAIN_BOARD wave_v3 H2 → Phase C
  NULL was the issue → paris promotes panel_v2_no_phase_c to production
- If still < ledashi 228 → more cols at fault, paris does 2nd ablation round
"""
...
PANEL_V2_NO_PHASE_C = "D:/dev/aurumq-handoffs/inbox/2026-05-15-paris-panel-v2-no-phase-c/combined_panel_v_x_v2_no_phase_c.parquet"
...
OUT_DIR = Path("data/kronos/outputs/matrix_v5")
...
# 7 cells: 4 CSI1000 × waves + MAIN_BOARD v3 v2 + MAIN_BOARD v3 ledashi + NPF v3 v2
CELLS = [
    ("v1", "CSI1000", "v2_no_phase_c"),
    ("v2", "CSI1000", "v2_no_phase_c"),
    ("v3", "CSI1000", "v2_no_phase_c"),
    ("v4", "CSI1000", "v2_no_phase_c"),
    ("v3", "MAIN_BOARD", "v2_no_phase_c"),
    ("v3", "MAIN_BOARD", "ledashi"),
    ("v3", "NPF", "v2_no_phase_c"),
]
...
def load_panel(name):
    if name == "v2_no_phase_c":
        print(f"[panel] reading {PANEL_V2_NO_PHASE_C} ...", flush=True)
        t = time.time()
        p = pd.read_parquet(PANEL_V2_NO_PHASE_C)
        p = _dt(p)
        drop_cols = [
        ...
        print(f"  panel v2_no_phase_c: {len(p):,} rows × {len(p.columns)} cols ({time.time()-t:.0f}s)")
        return p
    elif name == "ledashi":
...
def main():
    ...
    out = {
        "config": {
            "tier": "v5 — paris panel_v2_no_phase_c ablation (366 cols, drop 13 Phase C + 2 Kronos)",
            "cells": CELLS,
            "panel_v2_no_phase_c": PANEL_V2_NO_PHASE_C,
    ...
    Path("data/kronos/outputs/matrix_v5_results.json").write_text(json.dumps(out, indent=2, default=str))

    lines = ["# Matrix v5 — paris panel_v2_no_phase_c ablation", ""]
    lines.append(f"_Generated {time.strftime('%Y-%m-%d %H:%M Asia/Shanghai')}  runtime {time.time()-t_total:.0f}s_")
    lines.append("")
    lines.append("**Panel**: paris combined_panel_v_x_v2_no_phase_c (366 cols, drops 13 Phase C + 2 Kronos cols)")
    lines.append("**Hyperparam**: `n_estimators=200` fixed (no early-stop, per matrix v4 fix)")
    lines.append("")
    ...
    lines.append("")
    lines.append("## paris Q3 ablation answer")
    lines.append("")
    lines.append("Comparison MAIN_BOARD wave_v3 H2 fwd20 IC %:")
    lines.append("")
    lines.append("| panel | H2 fwd5 IC% | H2 fwd20 IC% | Q1 fwd20 IC% |")
    lines.append("|---|---|---|---|")
    ledashi = matrix_results.get("v3_MAIN_BOARD_ledashi", {})
    v2nopc = matrix_results.get("v3_MAIN_BOARD_v2_no_phase_c", {})
    def fmt_w(d, w, k="fwd20"):
        v = d.get("static", {}).get(w, {}).get(k, {}).get("ic", float("nan"))
        return f"{v*100:+.3f}" if v == v else "n/a"
    lines.append(f"| paris v2 NO Phase C (366 cols) | {fmt_w(v2nopc,'H2_2025','fwd5')} | {fmt_w(v2nopc,'H2_2025')} | {fmt_w(v2nopc,'Q1_2026')} |")
    lines.append(f"| ledashi 228 pruned (matrix v5 baseline) | {fmt_w(ledashi,'H2_2025','fwd5')} | {fmt_w(ledashi,'H2_2025')} | {fmt_w(ledashi,'Q1_2026')} |")
    lines.append("")
    lines.append("Matrix v4 reference (panel v2 WITH Phase C NULL):")
    lines.append("")
    lines.append("| panel | H2 fwd5 IC% | H2 fwd20 IC% | Q1 fwd20 IC% |")
    lines.append("|---|---|---|---|")
    lines.append("| paris v2 WITH Phase C NULL (381 cols, matrix v4) | +3.107 | +3.080 | +0.288 |")
    lines.append("| ledashi 228 pruned (matrix v4 baseline) | +2.868 | +4.143 | +1.713 |")

    Path("data/kronos/outputs/matrix_v5_table.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"\n[saved] matrix_v5_results.json + matrix_v5_table.md + {len(matrix_results)} predictions")
