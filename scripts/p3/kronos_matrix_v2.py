"""Matrix v2 — 8 universe × 4 labels × panel_only × 2022+ window.

Per ledashi PLAN.md 2026-05-14 (paris REPLY v15 confirmed all 5 designs):
- Universe: 8 (skip GROWTH_BOARDS, wave label MAIN_BOARD-only)
- Label: wave_v1 / wave_v2 / wave_v3 / wave_v4
- Feature: panel_only (228-col pruned, my local; NO Kronos/tech_lines panel feature)
- Train window: 2022-01-01 ~ 2024-12-31 (paris-confirmed as ablation, not production)
- Eval windows: H1_2025 / H2_2025 / Q1_2026 / Q2_2026_partial
- Static horizons: fwd1 / fwd3 / fwd5 / fwd10 / fwd20 / fwd30 (drop fwd15)
- Dyn exit (11 triggers): A_stop_5pct / C_vol_drop / E_trail_5pct / F_trend_break /
                          G_K_max / H_macd_death / I_kdj_death / S_ma5_below_ma10 /
                          J_take_profit_5 / J_take_profit_10 / Q_OR_FIE

Total: 32 LGB trains, est ~25 min compute on 4070 + 32C/64G.
"""
...
OUT_DIR = Path("data/kronos/outputs/matrix_v2")
...
def load_universes() -> dict:
    """Load static + PIT universe membership."""
    static_sets = {}
    pit_dfs = {}
    for u in UNIVERSES:
...
def filter_universe(panel: pd.DataFrame, name: str, static_sets, pit_dfs) -> pd.DataFrame:
...
def compute_realized_and_sim() -> tuple[pd.DataFrame, pd.DataFrame]:
    """Realized fwdK + per (entry, ts_code) 11-trigger dyn-exit sim (with MACD/KDJ + MA5/MA10)."""
    t = time.time()
    ...
    print("[realized] merging tech_lines (MACD/KDJ) ...", flush=True)
    ...
    for sym, sub in p.groupby("ts_code"):
        ...
        # S: MA5 cross BELOW MA10 (wife's death cross)
        ...
        # H_macd_death: macd line cross BELOW signal
        ...
        # I_kdj_death: KDJ K cross BELOW D
        ...
        I_f = I_abs[fwd_idx]
        S_f = S_abs[fwd_idx]
        J5_f = cum_ret > 0.05    # take profit at +5%
        J10_f = cum_ret > 0.10   # take profit at +10%
        Q_f = F_f | I_f | E_f    # ensemble OR
...
def main() -> int:
    t_total = time.time()
    print("[load] universes ...", flush=True)
    ...
    out = {
        "config": {
            ...
            "feature_set": "panel_only_228cols",
            "panel_source": PANEL,
        },
        "results": matrix_results,
        "total_time_s": time.time() - t_total,
    }
    Path("data/kronos/outputs/matrix_v2_results.json").write_text(json.dumps(out, indent=2, default=str))

    # Markdown table
    lines = ["# Matrix v2 results", ""]
    lines.append(f"_Generated {time.strftime('%Y-%m-%d %H:%M Asia/Shanghai')}  runtime {time.time()-t_total:.0f}s_")
    lines.append("")
    lines.append(f"**Train window**: {TRAIN_START} ~ {TRAIN_END} (paris-confirmed ablation)")
    lines.append("**Feature**: panel_only (228 cols)")
    lines.append("")

    short_trig = {"A_stop_5pct":"A", "C_vol_drop":"C", "E_trail_5pct":"E", "F_trend_break":"F",
                  "G_K_max":"G", "H_macd_death":"H", "I_kdj_death":"I",
                  "S_ma5_below_ma10":"S", "J_take_profit_5":"J5", "J_take_profit_10":"J10",
                  "Q_OR_FIE":"Q"}
    ...
    lines.append("\n## Dynamic exit (NPF universe, panel_only — H1+H2 avg ann Sharpe)")
    for wname in ("H1_2025", "H2_2025", "Q1_2026"):
        lines.append(f"\n### {wname}")
        hdr = ["label"] + [short_trig[t] for t in TRIGGERS]
        lines.append("| " + " | ".join(hdr) + " |")
        lines.append("|" + "---|" * len(hdr))
        for label_v in LABELS:
            row = [f"wave_{label_v}"]
            r = matrix_results.get(f"{label_v}_NPF", {})
            if r.get("skipped"):
                row += ["skip"] * len(TRIGGERS)
            else:
                for trig in TRIGGERS:
                    v = r.get("dynamic", {}).get(wname, {}).get(trig, {}).get("top50_sharpe", float("nan"))
                    row.append(f"{v:+.2f}" if v == v else "n/a")
            lines.append("| " + " | ".join(row) + " |")

    Path("data/kronos/outputs/matrix_v2_table.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"\n[saved] matrix_v2_results.json + matrix_v2_table.md + {len(matrix_results)} predictions")
