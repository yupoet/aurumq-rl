"""Wave v4 post-grid: ensemble + extend predictions + long-horizon eval."""
...
RUN_ROOT = Path("runs/sl_path1_long_wave_v4")
...
def step2_extend_predictions():
    ...
    target_y = pl.read_parquet(LONG / "target_y_wave_v4.parquet")
...
def step3_eval():
    ...
    extras = [
        ...
        ("Wave v4 (4-fix)", "runs/sl_path1_long_wave_v4/predictions.parquet", "score_calibrated"),
    ]
    for e in extras:
        if e not in ev2.PATHS: ev2.PATHS.append(e)
    ev2.OUT_ROOT = Path("runs/eval_matrix_v2_with_wave_v4")
    ev2.main()
    ...
    lh.OUT = Path("runs/eval_matrix_v2_with_wave_v4/long_horizon.md")
    lh.main()
