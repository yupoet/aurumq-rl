...
from aurumq_rl.labeling.events import Event, dedupe_events, derive_labels
...
def test_dedupe_non_overlapping_kept():
    e1 = Event(
        ts_code="A", event_start_idx=10, event_peak_idx=15, event_quality=1.0, event_method="A"
    )
    e2 = Event(
        ts_code="A", event_start_idx=20, event_peak_idx=25, event_quality=2.0, event_method="A"
    )
...
def test_dedupe_overlap_keeps_higher_quality():
    e1 = Event(
        ts_code="A", event_start_idx=10, event_peak_idx=20, event_quality=1.0, event_method="A"
    )
    # e2 starts at 15, which is <= e1.peak (20), overlap, e2 lower quality → keep e1
    e2 = Event(
        ts_code="A", event_start_idx=15, event_peak_idx=25, event_quality=0.5, event_method="A"
    )
    out = dedupe_events([e1, e2])
    assert len(out) == 1
...
def test_dedupe_overlap_keeps_higher_quality_swap():
    ...
    e2 = Event(
        ts_code="A", event_start_idx=15, event_peak_idx=25, event_quality=2.0, event_method="A"
    )
    out = dedupe_events([e1, e2])
    assert len(out) == 1
...
def test_dedupe_per_stock_isolation():
    ...
    e2 = Event(
        ts_code="B", event_start_idx=15, event_peak_idx=25, event_quality=1.0, event_method="A"
    )
...
def test_derive_labels_t1():
    ...
    e1 = Event(
        ts_code="A", event_start_idx=10, event_peak_idx=15, event_quality=2.0, event_method="A"
    )
...
def test_derive_labels_quality_threshold():
    ...
    ts_codes = ["A"]
    e_low = Event(
        ts_code="A", event_start_idx=10, event_peak_idx=15, event_quality=0.3, event_method="A"
    )
    e_high = Event(
        ts_code="A", event_start_idx=20, event_peak_idx=25, event_quality=2.0, event_method="A"
    )
